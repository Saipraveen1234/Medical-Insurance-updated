...
import {DialogBody, DialogFooter, DialogHeader, Dialog, Button} from "@material-tailwind/react";
...
export const StatisticsSection = ({ total2024, total2025, totalCombined }) => {
  ...
  return (
    <div className="mb-12 grid gap-y-10 gap-x-6 md:grid-cols-2 xl:grid-cols-3">
      {statisticsInsuranceData.map(({ icon, title, ...rest }) => (
        <StatisticsCard
          key={title}
          {...rest}
          title={title}
...
const InvoiceSummaryDashboard = () => {
  ...
  const { data: getUploadedData, loading: getUploadedLoading, error: getUploadederror, refetch } = useQuery(GET_UPLOADED_FILES, {
  ...
  if (!data?.getInvoiceData?.length) {
    return (
      <Paper sx={{ p: 3, mt: 2 }}>
        <Typography color="text.secondary" sx={{fontFamily: "Bookman Old Style"}}>
  ...
  return (
    <ThemeProvider theme={theme}>
      <CssBaseline/>
      <Container maxWidth="xl" sx={{ py: 3, bgcolor: "#f9fafb", minHeight: "100%"}}>
              <StatisticsSection
               total2024={total2024} 
               total2025={total2025} 
        ...
        {/* Filters */}
            ...
            <Grid item xs={12} md={4}>
              <Box sx={{ display: "flex", flexDirection: "column" }}>
                <Typography variant="body2" sx={{ mb: 1, fontWeight: 500, fontFamily: "Bookman Old Style"}}>
                  Plan Type
                </Typography>
                <ToggleButtonGroup
                value={planFilter}
              ...
              >
                <ToggleButton
                  value="ALL"
                  sx={{ flex: 1, py: 1, borderRadius: "4px 0 0 4px", fontSize: 14 ,fontFamily: "Bookman Old Style" }}
                >
                  ALL PLANS
                </ToggleButton>
                <ToggleButton
                  value="UHC"
                  sx={{ flex: 1, py: 1, borderRadius: 0, fontFamily: "Bookman Old Style", fontSize: 14 }}
                >
                  UHC
                </ToggleButton>
                <ToggleButton
                  value="UHG"
                  sx={{ flex: 1, py: 1, borderRadius: "0 4px 4px 0", fontFamily: "Bookman Old Style", fontSize: 14 }}
                >
                  UHG
                </ToggleButton>
              </ToggleButtonGroup>
              ...
              <Box sx={{ display: "flex", flexDirection: "column" }}>
                ...
                <FormControl fullWidth size="small">
                  <Select
                    ...
                    displayEmpty
                    sx={{ bgcolor: "white", borderRadius: "4px", fontFamily: "Bookman Old Style", padding: "1px" }}
                  >
                    <MenuItem value="ALL" sx={{fontFamily: "Bookman Old Style"}}>All Years</MenuItem>
                    {allFiscalYears.map((fy) => (
                      <MenuItem key={fy} value={fy.toString()} sx={{fontFamily: "Bookman Old Style"}}>
                        {fy}
                      </MenuItem>
                    ))}
                  </Select>
            ...
            <Button color="blue" style={{marginTop: "28px", textAlign: "left"}} onClick={handleOpen}>Datasets</Button>
            </Grid>
            </Grid>
          ...
          {currentItems.map(([key, group]) => {
            ...
            return (
              <Accordion
                ...
                sx={{ mb: 2, border: "1px solid #e5e7eb", bgcolor: "white", maxHeight: "80%", overflow: "scroll"}}
              >
                <AccordionSummary
                ...
                >
                  ...
                  >
                    <Typography variant="h6" sx={{ fontWeight: 600, fontFamily: "Bookman Old Style" }}>
                      {month} {year}
                    </Typography>
                    <Typography
                      variant="h6"
                            ...
                            {uhc.map((plan, idx) => {
                              ...
                              return (
                                <tr key={`uhc-${key}-${idx}`}>
                                  ...
                                  >
                                    <Typography sx={{fontFamily: "Bookman Old Style"}}>{formatPlanType(plan.planType)}</Typography>
                                  </td>
                                  <NumericCell
        ...
        <Dialog open={isModalOpen} handler={() => setIsModalOpen(false)} size="xl" className="p-5">
